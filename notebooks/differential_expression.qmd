...
results <- lapply(gene_libraries, function(lib) {
  run_enrichr(userListId, lib)
})
```
...
plot_barplot <- function(enrich_df, lib_name) {
  ...
  ggplot(enrich_df, aes(x = Term_wrapped, y = `Combined Score`)) +
...
enrich_df <- load_enrichment(library_name)
